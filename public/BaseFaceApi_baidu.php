...
class BaseFaceApi
{
    ...
    var $genders;   //性别
    var $ageLvels;  //年龄等级
    protected $face_field = "age,beauty,expression,face_shape,gender,glasses,landmark,race,quality,eye_status,emotion,face_type,quality";
    var $error;
     ...
     * @return array
     */
    public function faceRecognition($faceImg, $deviceCode, $storeId, $params = [], $apiType = 'baidu')
    {
...
//         $userId         = isset($params['user_id']) ? trim($params['user_id']) : 0;
        ...
        // 如果有可选参数
        $options = array();
        $options["face_field"] = $this->face_field;
        ...
        if ($deviceCode) {//设备上传图片
            $imgData = [
                ...
                'image_id'      => isset($detectResultBaidu['result']['face_list']['0']['id']) ? trim($detectResultBaidu['result']['face_list']['0']['id']) : '',
                'img_x'         => $faceX,
                'img_y'         => $faceY,
                'img_pixel'     => $imgPixel,
                'api_type'      => $apiType,
            ...
            if ($exist) {
                return ['code' => 0, 'face' => $faces[0], 'fuser_id' => $exist['fuser_id'], 'face_img' => $faceImg];
            }
        }
