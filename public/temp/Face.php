...
//        $result = $this->returnAndContinue($data);
...
//            'return_params' => $result,
        ...
        $results = @curl_post('http://bi.micyi.com/api/face/index',$this->postParams);
        exit();
    }
...
//        $this -> returnAndContinue(['code' => 0, 'msg' => 'ok']);
        ...
        $date = date('Ym');
        file_put_contents('../runtime/log/face' . $date,$str . PHP_EOL, FILE_APPEND);
        $deviceCode = $params['device_id'] ?? '';
        $command = $params['command'] ?? '';
        $eventTime = $params['event_time'] ?? '';
        $msgId = $params['msg_id'] ?? '';
        $type = $params['type'] ?? '';
        $faceData = $params['data']['face_list'] ?? '';
        $faceImg = $faceData[0]['face'] ?? '';
        $bigImg = $faceData[0]['body'] ?? '';
        $faceId = $faceData[0]['face_id'] ?? '';

        $left = $faceData[0]['face_rectangle']['left'] ?? ''; //矩形框左上角像素点的横坐标
        $top = $faceData[0]['face_rectangle']['top'] ?? ''; //矩形框左上角像素点的纵坐标
        $width = $faceData[0]['face_rectangle']['width'] ?? ''; //矩形框的宽度
        $height = $faceData[0]['face_rectangle']['height'] ?? ''; //矩形框的高度
        $face_x = $left + $width/2;
        $face_y = $top + $height/2;
        $imgPixel = $width . '*' . $height; //分辨率
        ...
        $dimgExist = $deviceFaceModel->where($imgData)->find();
        if($dimgExist || (cache('img') == $faceImg)){
            $this->_returnMsg(['code' => 1, 'msg' => 'face_img:图片重复']);
        }
        cache('img',$faceImg,360);
        $faceApi = new \app\common\api\BaseFaceApi();
        $params = [
            'capture_time'  => $eventTime,
            'face_x'        => $face_x,
            'face_y'        => $face_y,
            'img_pixel'     => $imgPixel,
            'block_id'      => $blockId,
        ...
        $result = $faceApi->faceRecognition($faceImg, $deviceCode, $storeId, $params);
        if ($result['code'] > 0) {
            $this->_returnMsg($result);
        }
