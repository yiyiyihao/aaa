...
//        if ($ret) {
//            $count = db('store_member') -> alias('SM') -> where($this->_getWhere($params,1))-> where([['SM.member_id','in',$ret]])->join($this ->_getJoin($params)) -> count();
//            $data = db('store_member') -> alias('SM') -> where($this->_getWhere($params,1)) ->where([['SM.member_id','in',$ret]])-> field($this->_getField($params)) -> join($this ->_getJoin($params)) ->order('U.group_id ASC, U.sort_order ASC, U.add_time DESC') -> limit(($page-1)*$size,$size) -> select();
//            if(!empty($data)) {
//                foreach ($data as $key => $value) {
//                    $where = [
//                        ['ML.member_id','=',$value['member_id']],
...
//        }
    ...
    public function related()
    {
        ...
        if($isRelated == 1){
            $resul = db('member_label') -> insert(['member_id'=>$memberId,'label_id'=>$labelId]);
            if(!$resul){
                $this->_returnMsg(['code' => 1, 'msg' => '关联失败']);die;
            }
            $re = db('label') -> where('label_id','=',$labelId) -> setInc('quote',1);
        }elseif($isRelated == 0){
            $result = db('member_label')->where(['member_id'=>$memberId,'label_id'=>$labelId])->update(['is_del'=>1]);
            if(!$result){
