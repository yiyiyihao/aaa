...
//         $this->date = date('Y-m-d',strtotime("-2 day"));
    ...
    public function getNormalDataset($count = 1) {
        ...
        $data = [
            ...
            'aveTimeline'   => rand(111,254) * 0.01,    //平均停留时长(分钟)
        ];
        return $data;
    }
    /**
     ...
     * 获取客户列表
     */
    public function getPersonDatalist($storeId = 0, $userType = 0, $date = FALSE, $field = FALSE,$page=1,$size=10, $flag = FALSE, $dateTime=''){
        $date = $date ? $date : $this->date;
        $list = $this->_getVisits($storeId, $userType, $date, $field,$page,$size, $flag, $dateTime);
        return $list;
    }
...
//         pre($list);
    ...
    public function getWeekDataset($storeId = 0, $genderList = false, $agelevelList = false, $ethnicityList = false,$days = 7,$endDate=''){
        $legend     = [];
        $dataset    = [];
        if($days == 1 && $endDate == date('Y-m-d')){
            $label = [];
            $timeNow = strtotime(date('Y-m-d H:00:00'));
            $timeStart = strtotime(date('Y-m-d 00:00:00'));
            $stop = ($timeNow - $timeStart)/3600;
            for ($i=0; $i < $stop; $i++) {
                $label[] = date('H:i',$timeStart+3600*$i);
            }
            if(time() > $timeNow){
                $label[] = date('H:i',time());
            }
        }elseif($endDate <> date('Y-m-d')){
            $label = $this->getDayList(24, 'hour', 'H:i', date('Y-m-d'));
        }else{
...
//            ];
        ...
        foreach ($label as $key => $value) {
            ...
            if($days == 1){
                $tempWhere = $where;
                $tempWhere[] = ['capture_time','BETWEEN',[strtotime($endDate . ' ' .$value)-3600,strtotime($endDate . ' ' .$value)]];
...
//             $timeNow = strtotime("-".$i." ".$type);
     ...
     * 获取满足条件的用户访问列表
     */
    private function _getVisits($storeid = 0, $userType = 0, $date = FALSE, $field = FALSE,$page=1,$size=10, $flag = FALSE, $dateTime='')
    {
        if ($this->dayVisits) {
...
//        }
        ...
        if($dateTime == '5min'){
            $timeStamp = 300;
            $where[] = ['DV.recent_time','>',time()-$timeStamp];
...
//             $this->dayVisits = db('day_visit')->field($field)->alias('DV')->join('store_member SM', 'DV.fuser_id = SM.fuser_id AND SM.store_id = '.$this->storeId, 'LEFT')->join('user_grade UG','UG.grade_id = SM.grade_id','LEFT')->join('store S','S.store_id = DV.store_id','LEFT')->where($where)->group('DV.fuser_id')->limit(($page-1)*$size,$size)->order('DV.capture_time desc')->select();
            ...
            $this->dayVisits = db('day_visit')->field($field)->alias('DV')->join('store_member SM', 'DV.fuser_id = SM.fuser_id', 'LEFT')->join('user U', 'U.user_id = SM.user_id', 'LEFT')->join('user_grade UG','UG.grade_id = SM.grade_id','LEFT')->join('store S','S.store_id = DV.store_id','LEFT')->where($where)->where('user_type', '<>', 3)->group('DV.fuser_id')->order('DV.recent_time desc')->limit(0,20)->select();
//             $this->dayVisits = db('day_visit')->field($field)->alias('DV')->join('store_member SM', 'DV.fuser_id = SM.fuser_id AND SM.store_id = '.$this->storeId, 'LEFT')->join('user_grade UG','UG.grade_id = SM.grade_id','LEFT')->where($where)->where('user_type', '<>', 3)->order('DV.recent_time desc')->select();
            if ($this->dayVisits) {
    ...
    private function _doVisits($vists = FALSE,$temp = null){
        $vists = $vists ? $vists : $this->dayVisits;
        $faceApi =  new \app\common\api\FaceApi();
        $ageLevels = $faceApi->ageLvels;
        $list = [];
        if(!empty($vists)) {
            $block = db('store_block') -> where('is_del','=',0) -> where('store_id','IN',$this->storeId) -> column('block_id,name');
            foreach ($vists as $key => $value) {
                ...
                $vists[$key]['age_tag'] = $ageLevels[$value['age_level']]['tag'][$value['gender']];
...
//                }
                ...
                if(isset($vists[$key]['group_id']) && $temp){
                    $vists[$key]['role_name'] = empty($vists[$key]['is_admin']) ? "{$vists[$key]['realname']}(" . lang('会员') . ')' : "{$vists[$key]['realname']}(" . lang('员工') . ')';
                    $vists[$key]['role_type'] = empty($vists[$key]['is_admin']) ? "2" : "1";
                }
                elseif($vists[$key]['total_visit_counts'] > 1){
                    $vists[$key]['role_name'] = lang('访客');
                    $vists[$key]['role_type'] = '3';
                }
                else{
...
//        }
     ...
     * 获取设备统计，计算进店，离店统计
     */
    public function getTrendDataset($return = FALSE,$color = false, $countValue = 1, $setflag = false) {
        if (!$this->dayTotal) {
            $this->_getDayTotal();
        }
        $trend = $this->dayTotal && $this->dayTotal['device_json'] ? json_decode($this->dayTotal['device_json'], true): [];
        //计算计算进店，离店统计
        $data = [];
        $data[2]['value'] = $data[1]['value'] = $data[0]['value'] = 0;
        $data[2]['name'] = lang('其它');
        $data[1]['name'] = lang('离店');
        $data[0]['name'] = lang('进店');
        if(!empty($trend)){
            $deviceIds = array_keys($trend);
            $deviceInfo = db('device')
                -> where([
                    ['device_id','in',$deviceIds],
                ])->field('device_id,position_type')->select();
            foreach($trend as $k => $v){
                foreach($deviceInfo as $key => $value){
                    if($k == $value['device_id'] && $value['position_type'] == 1){
                        $data[0]['value'] = $v;
                    }elseif($k == $value['device_id'] && $value['position_type'] == 2){
                        $data[1]['value'] = $v;
                    }else{
                        $data[2]['value'] = $v;
                    }
