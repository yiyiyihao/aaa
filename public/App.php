...
//                'store_id'      => $storeId,
    ...
    protected function sendSms()
    {
        ...
        if ($type == 'activity') {
            ...
            if ($exist) {
                $this->_returnMsg(['code' => 1, 'join' => 1, 'msg' => '当前手机号已参与过抽奖活动，请使用其它号码参与']);
            }
        }
        //判断短信验证码发送时间间隔
    ...
    protected function faceDetect()
    {
        ...
        $upload = new \app\api\controller\v1\upload\Upload($request);
...
//        while (list ($key, $val) = each ($para)) {
    ...
    protected function joinActivity()
    {
        ...
        $exist = db('win_log')->where($data)->find();
        if ($exist){
            $this->_returnMsg(['code' => 1, 'msg' => '当前手机号码已参与过活动']);
        }
        $config = cache('win_data');
        ...
        if ($totalNum <= 0) {
            ...
            //获取已中奖数量
            $existNum = db('win_log')->where(['type' => 2, 'phone' => ['NEQ', ''], 'status' => ['NEQ', -1]])->count();
            ...
            if ($lastNum <= 0) {
                ...
                if ($dayTotalNum <= 0) {
                    ...
                    //获取今日已中奖数量
                    $dayBeginTime = mktime(0, 0, 0, date("m"), date("d"), date("y"));
                    $todayExistNum = db('win_log')->where(['type' => 2, 'phone' => ['NEQ', ''], 'status' => ['NEQ', -1], 'add_time' => ['>', $dayBeginTime]])->count();
        ...
        if (!$exist) {
            ...
            $result = db('win_log')->insertGetId($data);
        }elseif ($exist && $exist['status'] == -1 && $flag){
            $result = db('win_log')->where(['log_id' => $exist['log_id']])->update(['add_time' => time(), 'status' => 0]);
        }
