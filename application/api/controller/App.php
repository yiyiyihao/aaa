...
//                'store_id'      => $storeId,
    ...
    protected function getUserProfile($openid = '')
    {
        ...
        $shop_code = db('store')->where('store_id','=',$storeId)->value('shop_code');
        $cardNo = db('store_member')->where($where)->value('card_no');
        $profile = [
            'openid'    => $user['openid'],
            'zt_openid'    => $user['uopenid'],
            'shop_code'    => $shop_code,
            'card_no'   => trim($cardNo),
...
//         }
    ...
    private function _checkOpenid($openid = '')
    {
        ...
        $field = 'UD.*,U.openid as uopenid,U.fuser_id,U.username,U.realname,U.password,U.face_img,U.email,U.email,U.phone,U.balance,U.age,U.last_login_time,U.group_id,U.is_admin';
        $user = db('user_data')->alias('UD')->join('user U', 'UD.user_id = U.user_id', 'LEFT')->field($field)->where([['UD.openid' ,'=', $openid], ['UD.user_id' , '>', 0]])->find();
        if (!$user) {
